...
### in case there is a problem compiling the text files (the source commands above)
### then load these files in from output folder
### I encountered a problem with 2014 and 2015 due to "multibyte" text when switching to Mac
### I added code using iconv(x, from = "", to = "UTF-8) in 2014
### and also fileEncoding = "437" in 2015 
### might need to reverse or alter these to get them to work on other machines
...
```
# JAGS model for fish "reached" and "passed given reached" proportion (p that reach the ladder)
logit-normal with hyperparameter for each fish
```{r}
...
# Run passage model
```{r}
# add in 2018 reach & passage info (temporarily doing it this way to avoid messing up previous code)
fish_jags_data_16yr <- fish_jags_data
fish_jags_data_16yr$nfishyears <- 16
fish_jags_data_16yr$z <- c(fish_jags_data$z, 0)
fish_jags_data_16yr$N <- c(fish_jags_data$N, 0)
fish_jags_data_16yr$c <- c(fish_jags_data$c, 1)

fish_mod_new <- jagsUI::jags(
  data = fish_jags_data_16yr,
...
fish_mod_new
# saveRDS(fish_mod_new, "other/output/fish_jags_mod_02202024_logit_all_converged_16yr.rds")
```
# Run "reached" and "passed given reached" models 
fraction of fish that reached ladder, and that passed given that they reached it (shad only)
```{r}
...
fish_mod_reach
# saveRDS(fish_mod_reach, "other/output/fish_jags_mod_reach_022024_logit_all_converged_16yr.rds")
```
# Create Figure 4 - Plot model output - logit (2-panel version)
```{r}
# new version with 1 herring passed
fish_mod <- readRDS(here(
  "other",
  "output",
  "fish_jags_mod_060122a_logit_all_converged.rds"
))

# dev.new()
# plot(fish_mod)

# for 2018 shad n annotation
shad_n <-
  data.frame(
    i = 16,
    theta_mean = NA,
...
thetas <- tidybayes::gather_draws(fish_mod, p[i]) %>%
  ...
  mutate(year = c(2012:2017, 2019, 2012:2019),
         species = c(rep("American Shad", 7), rep("River Herring", 8))) %>%
  left_join(raw_sum2) %>%
...
thetas %>%
  mutate(yearmid = year,
         year = case_when(species == "River Herring" ~ year-0.15,
                          TRUE ~ year)) %>%
  ggplot() +
  geom_vline(
    data = omegas,
  ...
  geom_errorbar(aes(xmin = omega_q10,
                ...
                width = 0,
                data = shad_2018) +
  # add CI bars for the corrected passage rate
  geom_errorbar(aes(xmin = theta.cor_q2.5,
                    xmax = theta.cor_q97.5,
                    y = year+0.25), width = 0,
                data = herring.cor, color = "darkgray") +
  geom_errorbar(aes(xmin = theta.cor_q10,
                    xmax = theta.cor_q90,
                    y = year+0.25),
                size = 1,
                width = 0,
  ...
  geom_point(aes(y = year+0.25, x = theta.cor_mean),
             size = 4, data = herring.uncor, color = "darkgray") +
  ...
  geom_point(aes(x = prop, y = yearmid),
             shape = "x",
             color = "red",
             size = 4) +
  geom_text(aes(x = x, y = yearmid, 
                label = n_label),
            nudge_x = .02,
            color = "darkgray",
            hjust = 1)
...
# ggsave(
#   "other/figures/Figure4_20240219d.png",
#   width = 7,
#   heigh = 5,
#   dpi = 600
# )
# fig previously named: jags_nofishhypers_logit_allconverged_run060122a.png
```
# Create Figure 4 - Plot model output - logit (3-panel version)
```{r}
# new version with 1 herring passed
fish_mod <- readRDS(here(
  "other",
...
# replace NA 2018 values for American Shad with overall mean (omega) values
thetas3[thetas3$year==2018 & thetas3$species=="American Shad",]$theta_mean <- shad_2018$omega_mean
thetas3[thetas3$year==2018 & thetas3$species=="American Shad",]$theta_q2.5 <- shad_2018$omega_q2.5
thetas3[thetas3$year==2018 & thetas3$species=="American Shad",]$theta_q97.5 <- shad_2018$omega_q97.5
thetas3[thetas3$year==2018 & thetas3$species=="American Shad",]$theta_q10 <- shad_2018$omega_q10
thetas3[thetas3$year==2018 & thetas3$species=="American Shad",]$theta_q90 <- shad_2018$omega_q90
...
ggsave(
  "other/figures/Figure4_20240219e.png",
  width = 10.5,
  heigh = 5,
  dpi = 600
)
# fig previously named: jags_nofishhypers_logit_allconverged_run060122a.png
```
# Create Figure 5 - Run size based on video time series & passage rates
```{r}
...
# ggsave("other/figures/Figure5.png",
#        height = 4, width = 7,
#        dpi = 600)

# figure previously named: run_size_passage_cor_uncertainfallback_run060122a_logit_20220601_newdata.png

# get corrected passage rate
omega_draws %>% 
  filter(species == "River Herring",
  year == 2021) %>%
  group_by(species, year) %>% 
...
```
# Create Figure 2 - Attraction, staging, passage times
```{r}
...
ggsave("other/figures/Figure2.png",
       height = 4, width = 7,
       dpi = 600)

# figure previously named: passage_time_boxplot4.png
```
...
```
# Create Figure 3 - Stairstep figure
```{r}
...
# ggsave("other/figures/Figure3.png", width = 7, height = 5, dpi = 600)

# figure previously named: stairstep_20220601.png
```

# Histogram of fish counts timing (by hour)
```{r}
...
```
# Create Figure S1 - Histogram of fish counts timing relative to sunrise/sunset
```{r}
...
# ggsave("other/figures/FigureS1.png",
#        width = 7, height = 4, dpi = 600)

# figure previously named: fish_counts_by_relative_sr_ss_histograms_axlabs4.png
p.day.plot %>% filter(bin %in% seq(100,160, 10)) %>% group_by(species, y2013) %>% summarise(sump = sum(p))

p.day.plot %>% filter(bin %in% seq(100,160, 10)) %>% group_by(species, y2013) %>% summarise(sump = sum(p))
```
# calculate metrics about when fish pass
For reference: in 2013 there was 1 fish (0.2%) counted > 60 min after sunset; 6 (2%) counted >45 min after sunset;  14 (4%) counted >30 min after; and 27 (8%) counted anytime after sunset.
```{r}
# American Shad
...
### special code to plot 2012 and 2014 together (A4 antenna only)
...
(   antenna_plot <-
 ...
 ggsave(here("figures", paste0("2012_2013_antenna_pct_hrs_active.png")), 
         height = 4, width = 7, dpi = 600)

```

# % hours at least 1 antenna active during peak fish time
```{r}
...
```
# Create Figure SX - Plot "attraction" model output - logit (2-panel version)
```{r}
# new version with 1 herring passed
...
ggsave(
  "other/figures/FigureSx_attraction_rate_model_20240220d.png",
  width = 7,
  heigh = 5,
  dpi = 600
)
```
# Create Figure SX - Plot "passage given attraction" model output - logit (just shad)
```{r}
# new version with 1 herring passed
fish_mod_reach <- readRDS(here(
  "other",
...
ggsave(
  "other/figures/FigureSx_passage_given_attraction_rate_model_20240220e.png",
  width = 4,
  heigh = 5,
  dpi = 600
)
