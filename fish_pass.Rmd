...
f <- tags %>%
  ...
  mutate(passage.days = case_when(passage.days > 300 ~ NaN,
                                  TRUE ~ passage.days)) %>%
...
```
# Plot model output - logit, 052022k
```{r}
...
ggsave(
  "other/figures/jags_nofishhypers_logit_allconverged_run060122a.png",
  width = 7,
  heigh = 5,
  dpi = 400
)
...
ggsave("other/figures/run_size_passage_cor_uncertainfallback_run060122a_logit_20220601_newdata.png",
       height = 4, width = 7,
       dpi = 400)
...
# ggsave("figures/stairstep_20220601.png", width = 7, height = 5, dpi = 400)
...
```
# Histogram of fish counts timing relative to sunrise/sunset
```{r}
...
ggplot(p.day.plot) +
  ...
  theme(text = element_text(size = 14),
        strip.background = element_rect(fill = "white"),
        axis.text.x = element_blank()) +
  labs(x = "Time relative to sunrise and sunset", y = "% of individuals counted")

ggsave("figures/fish_counts_by_relative_sr_ss_histograms.png", 
       width = 7, height = 4, dpi = 400)
